...
CLASS zpl_pxq_f01 IMPLEMENTATION.
  ...
  METHOD if_rsplfa_srvtype_imp_exec_ref~execute.
    ...
    DATA: lr_struc        TYPE REF TO data,
          ...
          wa_result       TYPE zpl_utiles=>ty_wa_result_pxq,
          lt_result       TYPE zpl_utiles=>ty_lt_result_pxq,
...
*        l_r_param_elem = i_r_param_set->get_param_elem( 'ZTABLA' ).
*        l_r_param_elem->get_value( IMPORTING e_value = l_tabla  ).

* toma version de zpl_utiles
        l_tabla = lo_utiles->c_plan.
        l_version = lo_utiles->c_version.
...
*          CHECK l_status EQ 1. Se comenta este check, ya que la precarga "ACTIVA" la
...
*       FREE c_th_data[].
          ...
          LOOP AT lt_result ASSIGNING <r>.
            ...
            LOOP AT lo_utiles->gt_fields_pxq INTO DATA(wa_field).
