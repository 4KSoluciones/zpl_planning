*&---------------------------------------------------------------------*
*&  Include  zpl_if_o02_end_v1 - Variable
*&---------------------------------------------------------------------*
*
...
*2) VENTAS REALES Agrupadas por Linea, Of Vta y Marca - Fuente Reales CEBE (ZPCA_O01N)
...
*     Inicializa wa_result:
         ...
         wa_result-version = o_utiles->g_draft_version.
...
*            Busca las ventas agrupadas por sucursal.  Solo para Cuentas que NO están en Excepción
...
*Venta COPA Presupuesto 23V0
             ...
             IF sy-subrc EQ 0.
               wa_result-/bic/zpl_venta = <fs_venta_pre>-ventastotal * 100.
...
*          Busca las ventas agrupadas por Linea y Sucursal
               READ TABLE lt_vta_pres_lin_suc ASSIGNING FIELD-SYMBOL(<fs_suc_lin_pre>) WITH KEY comp_code = <fs_clave>-comp_code
                                                                            ...
                                                                            BINARY SEARCH.
               IF sy-subrc EQ 0.
                 wa_result-/bic/zpl_venta = <fs_suc_lin_pre>-ventastotal * 100.
               ELSE.
...
*----------------------------------------*
           ...
           IF sy-subrc EQ 0.
             ...
             wa_result-/bic/zpl_gastp =  abs( <fs_presup>-gastostotal ) * wa_result-/bic/zpl_coef."
           ELSE.
             wa_result-/bic/zpl_gastp = 1.

*          Busca las ventas agrupadas por Linea y Sucursal
             READ TABLE lt_vta_pres_lin_suc ASSIGNING <fs_suc_lin_pre> WITH KEY comp_code = <fs_clave>-comp_code
                                                                          fiscper   = <fs_perio>-plan
                                                                          sucursal  = <fs_cost>-sales_off
                                                                          linea     = <fs_cost>-matl_group
                                                                          BINARY SEARCH.
             ...
             ELSE.
*            Busca las ventas agrupadas por sucursal
               READ TABLE lt_vta_pres_suc ASSIGNING <fs_suc_pre>  WITH KEY comp_code = <fs_clave>-comp_code
                                                                            fiscper   = <fs_perio>-plan
                                                                            sucursal  = <fs_cost>-sales_off
                                                                            BINARY SEARCH.
               IF sy-subrc EQ 0 AND <fs_clave>-gl_account NOT IN o_utiles->gt_cuentas.
